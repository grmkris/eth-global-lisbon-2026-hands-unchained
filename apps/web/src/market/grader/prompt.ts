...
export const gradingPrompt = (row: LedgerRow): string =>
	[
		...
		"`inputPackets` counts only packets that CARRIED motion — an operator who",
		"holds still sends none, however long they stay connected. So packets and",
		"commandedMotion rise and fall together, and a high packet count with",
		"commandedMotion near zero is not a thing you should ever see; treat it as",
		"a malformed stream rather than as work.",
		"With recordedEpisode null: episodeSaved true plus commandedMotion above",
		"~10 over a 5-300s window is real work — score it 70+. Fail a success",
		"claim whose commandedMotion is ~0, whatever else is set: an episode file",
		"can exist and still contain a motionless arm.",
